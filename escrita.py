...
class Escrita:
    ...
    def capturarProcessos(self):
        ...
        consumoCPUProcesso = [
            (proc.info.get("cpu_percent", 0)) / num_cpus for proc in processos_info
        ]

        consumoRAMProcesso = [
            (
                (
                    proc.info.get("memory_full_info").uss
                    if proc.info.get("memory_full_info")
                    else 0
                )
                / total_ram
            )
        ...
        for i in range(len(idProcessos)):
            processos_dict = {
                ...
                "Data": horas,
                "idProcessos": idProcessos[i],
                "nomeProcesso": nomeProcesso[i],
                "usuarioProcesso": usuarioProcesso[i],
                "consumoCPUProcesso": consumoCPUProcesso[i],
                "consumoRAMProcesso": consumoRAMProcesso[i],
            }
    ...
    def obterInformacoesComponentes(self):
        ...
        return {
            ...
            "velocidadeEscrita": [velocidade_escrita],
            "velocidadeLeitura": [velocidade_leitura],
            ...
            "conexoesAtivas": [conexoes_ativas],
        }
