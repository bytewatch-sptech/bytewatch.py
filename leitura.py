...
class Leitura:
    ...
    def __init__(self):
        ...
        self.bucket = "amzn-s3-bucket-teste-projeto"
    ...
    def salvarArquivoNoBucket(self, file, bucket, path, fileName):
        ...
        s3.upload_file(Filename=f"{file}", Bucket=f"{bucket}", Key=f"{path}/{fileName}")
    ...
    def agrupar_processos_por_nome(self):
        ...
        dataframeProcessos = pd.DataFrame.from_dict(
            agrupado, orient="index"
        ).reset_index()
    ...
    def formatarDadosComponentes(self):
        ...
        droppedPackets = self.ultimo_dado.get("droppedPackets", 0)
        # conexoesAtivas = self.ultimo_dado["conexoesAtivas"]
        conexoesAtivas = self.ultimo_dado.get("conexoesAtivas", 0)
        ...
        megabytesEnviados = round(self.ultimo_dado.get("bytesEnviados", 0) / 1024**2, 2)
        megabytesRecebidos = round(
            self.ultimo_dado.get("bytesRecebidos", 0) / 1024**2, 2
        )
        velocidadeDownload = round(
            self.ultimo_dado.get("velocidadeDownload", 0) / 1024**2, 2
        )
        velocidadeUpload = round(
            self.ultimo_dado.get("velocidadeUpload", 0) / 1024**2, 2
        )

        velocidadeEscrita = round(
            self.ultimo_dado.get("velocidadeEscrita", 0) / 1024**2, 2
        )
        velocidadeLeitura = round(
            self.ultimo_dado.get("velocidadeLeitura", 0) / 1024**2, 2
        )
        ...
        dados_resultados = {
            ...
            "velocidadeEscrita": [velocidadeEscrita],
