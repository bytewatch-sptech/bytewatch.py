...
class Client:
    ...
    def carregarArquivos(self):
        try:
            ...
            self.df_processos = pd.read_csv(
                "processos_trusted.csv", on_bad_lines="skip"
            )
    ...
    def adicionar_alerta(
        self, mac, componente, valor, horario, historico_valores, historico_momentos
    ):
    ...
    def dashboardAlertasGestor(self):
        self.conteudo = {"alertas": [], "historico": []}
        ...
        for mac in self.df_metrica["macAddress"].unique():
            ...
            momentos_reciprocos = (
                df_maquina["horario"].tail(7).dt.strftime("%H:%M").tolist()
            )
            ...
            self.adicionar_alerta(
                mac,
                "ram",
                ultima_linha.porcentagemRam,
                ultima_linha.horario,
                hist_ram,
                momentos_reciprocos,
            )
            self.adicionar_alerta(
        ...
        if not df_24h.empty:
            df_24h["max_uso"] = df_24h[
                ["cpuPorcentagem", "porcentagemRam", "porcentagemDisco"]
            ].max(axis=1)

            qtd_critico = len(df_24h[df_24h["max_uso"] >= 90])
            qtd_alerta = len(
                df_24h[(df_24h["max_uso"] >= 70) & (df_24h["max_uso"] < 90)]
            )
            ...
            if not df_problemas.empty:
                ...
                df_problemas["is_alerta"] = (
                    (df_problemas["max_uso"] >= 70) & (df_problemas["max_uso"] < 90)
                ).astype(int)
                df_problemas["is_total"] = 1

                df_volatilidade = (
                    df_problemas.groupby("macAddress")
                    .agg(
        ...
        for mac in self.df_metrica["macAddress"].unique():
            ...
            self.conteudo["ServidorEmPico"] = {
                "servidorPicoRAM": {
                    ...
                    "ultimaColeta": ultima_captura.horario.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                },
                "servidorPicoCPU": {
            ...
            for posicao, (mac, pico) in enumerate(ranking_ram_ordenado, start=1):
                ranking_formatado.append(
                    {"posicao": posicao, "macAddress": mac, "picoRam": pico}
                )
    ...
    def dashboardServidoresGerais(self):
        ...
        for mac in self.df_metrica["macAddress"].unique():
            df_maquina = self.df_metrica[
                self.df_metrica["macAddress"] == mac
            ...
            status_rede = (
                "critico"
                if rede_eficiencia < 70
                else ("atencao" if rede_eficiencia < 85 else "estavel")
            )
            ...
            if (
                "critico" in status_componentes
                or status_componentes.count("risco") >= 2
            ):
...
>>>>>>> 97e1e13c8dc8e1b920667e4ef33648219ae84e48
            ...
            eixo_x = (
                ...
                + (ram_val * 0.30)
                + (disco_val * 0.10)
                + (rede_eficiencia_invertida * 0.10)
            )
    ...
    def dashboardCpu(self):
        ...
        for mac in self.df_metrica["macAddress"].unique():
            ...
            nucleos_logicos = (
                int(ultima_linha["cpuNucleosLogicos"])
                if ultima_linha["cpuNucleosLogicos"] > 0
                else 1
            )
            ...
            self.conteudo[mac]["metricas"] = {
                ...
                "kpiInformacao": {
                    "macAddress": mac,
                    "ultimaColeta": ultima_linha["horario"].strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    "nomeMaquina": ultima_linha["nome_maquina"],
                    "processador": ultima_linha["processador"],
            ...
            for nome in processos_unicos:
                ...
                dado_processo = {
                    ...
                    "quantidadeProcessos": int(
                        ultima_linha_proc["quantidadeProcessos"]
                    ),
    ...
    def dashboardEquipe(self):
        if self.df_metrica.empty:
            return

        self.conteudo = {"geral": {}, "por_servidor": {}}
        ...
        if df_7d.empty:
            self.salvarArquivo("dashboard_equipe_tela.json")
            return

        def classificar_alerta(row):
            max_val = max(
                row["cpuPorcentagem"], row["porcentagemRam"], row["porcentagemDisco"]
            )
            if max_val >= 90:
                return "Crítico"
            elif max_val >= 70:
        ...
        df_7d["severidade"] = df_7d.apply(classificar_alerta, axis=1)

        df_alertas = df_7d[df_7d["severidade"] != "Normal"].copy()
        df_alertas["dia_semana"] = df_alertas["horario"].dt.dayofweek

        def montar_bloco_grafico(df_filtrado):
            dias_rotulos = ["Seg", "Ter", "Qua", "Qui", "Sex", "Sáb", "Dom"]
            tratados = [0] * 7
            criticos = [0] * 7

            if not df_filtrado.empty:
                agrupado = (
                    df_filtrado.groupby(["dia_semana", "severidade"])
                    .size()
                    .unstack(fill_value=0)
                )
                for i in range(7):
                    if i in agrupado.index:
                        if "Aviso" in agrupado.columns:
                            tratados[i] = int(agrupado.loc[i, "Aviso"])
                        if "Crítico" in agrupado.columns:
                            criticos[i] = int(agrupado.loc[i, "Crítico"])

            return {
                "categorias": dias_rotulos,
                "series": [
                    {"nome": "Alertas Tratados", "dados": tratados},
                    {"nome": "Alertas Críticos", "dados": criticos},
                ],
            }

        self.conteudo["geral"] = montar_bloco_grafico(df_alertas)

        if not df_alertas.empty:
            for mac in df_alertas["macAddress"].unique():
                df_mac = df_alertas[df_alertas["macAddress"] == mac]
                self.conteudo["por_servidor"][mac] = montar_bloco_grafico(df_mac)
